...
# Slurm parameters
#SBATCH --job-name=analyze_activity_echi
...
#uv run --extra=cu118 audio_test_EchoSet.py --conf_dir /misc/usrhomes/s1495/TIGER/Experiments/checkpoint/CausalTIGER3-on-EchoSet/conf.yml --save_dir /no_backups/s1495/TIGER/CausalTIGER3-on-EchoSet
#uv run --extra=cu118 audio_test_ECHI.py --conf_dir /misc/usrhomes/s1495/TIGER/Experiments/checkpoint/TIGER-TSE2-ECHI_without-Noise_with_standard_PIT/conf.yml --save_dir /no_backups/s1495/TIGER/TIGER-TSE2-ECHI_without-Noise_with_standard_PIT
...
#uv run visualize_reports_matplotlib.py \
 ...
 # --metrics se_sisdr_all se_sisdr_all_i sisdr_active sisdr_active_i residual_loss \
  #--dpi 300
uv run --extra=cu118 analyze_echi_active_speakers.py configs/tiger_tse2.yml --activity_tau 1e-6 --out_csv echi_active.csv

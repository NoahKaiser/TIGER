...
following adjustments are made to achieve a causal tiger model:
 ...
 4) replaces GroupNorm from Band-SplitModule (self.BN) with CumulateLN
 5) STFT/iSTFT changed to low-latency causal framing (center=False, configurable analysis/synthesis windows, explicit look_ahead)
"""
...
class PerFrameFreqSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Concatenate heads along batch dimension:
        # Shapes:
        #   Q: [B*H, E, T, F]
        ...
        # fold time into batch: (B*H, E, T, F) -> (B*H*T, F, E)
    ...
    def __init__(
        ...
        self.freq_path = nn.ModuleList([
            ...
            PerFrameFreqSelfAttention(
                ...
                n_freqs=1,  # keep consistent with current LayerNormalization4D config usage
            ),
...
class CausalTIGER4(BaseModel):
    def __init__(
        ...
        super(CausalTIGER4, self).__init__(sample_rate=sample_rate)
        ...
        if self.analysis_win > self.win:
            raise ValueError("analysis_win must be <= n_fft (win).")
        if self.synthesis_win > self.win:
        ...
        """replaced nn.Sequential(nn.GroupNorm(1, self.band_width[i]*2, self.eps) with CumulateLN(2*self.band_width[i], eps=self.eps, affine=True)"""
        self.BN = nn.ModuleList([])
        for i in range(self.nband):
            self.BN.append(nn.Sequential(normalizations.CumulateLN(2*self.band_width[i], eps=self.eps),
    ...
    def pad_input(self, input):
        """
        Causal-safe right padding for center=False STFT and explicit look-ahead.
        """
        if self.look_ahead > 0:
            input = F.pad(input, (0, self.look_ahead))

        num_samples = input.shape[-1]
        if num_samples < self.win:
            rest = self.win - num_samples
        else:
            rest = (self.stride - ((num_samples - self.win) % self.stride)) % self.stride
